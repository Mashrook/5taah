...
// Auth helper - validates JWT and returns user_id or null
...
async function searchFlights(params: {
  ...
  if (params.airline) qs.set("includedAirlineCodes", params.airline);
...
async function bookTransferOffer(params: {
  ...
  const requestBody = {
    data: {
      ...
      agency: {
        contacts: [{ email: { address: "bookings@khattah.com" } }],
      },
    },
  };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET" && action === "search") {
      ...
      const travelClass = url.searchParams.get("travelClass") || undefined;
      ...
      const data = await searchFlights({ origin, destination, departureDate, returnDate, adults, max, travelClass, airline });
    ...
    if (req.method === "POST" && action === "book") {
      ...
      console.log(`[Amadeus] Flight booking by user: ${userId}`);
    ...
    if (req.method === "POST" && action === "hotel-book") {
      ...
      console.log(`[Amadeus] Hotel booking by user: ${userId}`);
    ...
    if (req.method === "POST" && action === "transfer-book") {
      ...
      console.log(`[Amadeus] Transfer booking by user: ${userId}`);
    ...
    if (req.method === "POST" && action === "flight-confirm") {
      const body = await req.json();
      const { paymentSessionId, paymentId } = body;
      if (!paymentSessionId) {
        return new Response(
          JSON.stringify({ error: "paymentSessionId required" }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      ...
      const details = (session as Record<string, unknown>).details_json as Record<string, unknown> || {};
      const travelers = (details.travelers || []) as Record<string, unknown>[];
      const firstTraveler = travelers[0] as Record<string, string> | undefined;
      ...
      const { data: booking, error: bookingError } = await supabase
        ...
        .insert({
          flow: "flight",
          status: "confirmed",
          amount: (session as Record<string, unknown>).amount || 0,
          currency: (session as Record<string, unknown>).currency || "SAR",
          ...
          user_id: (session as Record<string, unknown>).user_id || null,
          tenant_id: (session as Record<string, unknown>).tenant_id || null,
      ...
      if (bookingError) {
        console.error("[Amadeus] Booking creation failed:", bookingError);
        return new Response(
          JSON.stringify({ error: "Failed to create booking", detail: bookingError.message }),
    ...
    return new Response(
      JSON.stringify({ error: "Invalid action. Use: search, price, book, hotel-list, hotel-offers, hotel-offer-details, hotel-book, transfer-search, transfer-book, transfer-cancel, flight-confirm" }),
      { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error: unknown) {
