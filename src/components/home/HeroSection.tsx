...
export default function HeroSection() {
  ...
  return (
    <section className="bg-gradient-to-b from-primary/5 via-background to-background pb-8 pt-4">
      <div className="container mx-auto px-4 lg:px-8">
        {/* Hero Text */}
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground leading-tight mb-3">
            اكتشف وجهتك القادمة مع <span className="text-primary">خته</span>
          </h1>
            ...
            <div className="p-5">
              {searchTab === "flights" && (
                <>
                  ...
                  <div className={`grid grid-cols-2 ${tripType === "roundtrip" ? "lg:grid-cols-4" : "lg:grid-cols-3"} gap-3`}>
                    <CityAutocomplete value={fromCity} onChange={setFromCity} placeholder="من أين؟" showCode inputClassName="bg-muted/40 border-border text-sm text-right" />
                    <CityAutocomplete value={toCity} onChange={setToCity} placeholder="إلى أين؟" showCode inputClassName="bg-muted/40 border-border text-sm text-right" />
                    <DatePickerInput value={departDate} onChange={setDepartDate} placeholder="تاريخ الذهاب" disabled={disablePast} className="bg-muted/40 border-border text-sm" />
                    {tripType === "roundtrip" && (
                      <DatePickerInput value={returnDate} onChange={setReturnDate} placeholder="تاريخ العودة" disabled={(d) => disablePast(d) || (departDate ? d < new Date(departDate) : false)} className="bg-muted/40 border-border text-sm" />
                    )}
                  </div>
                </>
              )}
              ...
              {searchTab === "hotels" && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                  <HotelAutocomplete value={hotelCity} onChange={setHotelCity} placeholder="المدينة أو اسم الفندق" inputClassName="bg-muted/40 border-border text-sm text-right" />
                  <DatePickerInput value={checkIn} onChange={setCheckIn} placeholder="تسجيل الدخول" disabled={disablePast} className="bg-muted/40 border-border text-sm" />
                  <DatePickerInput value={checkOut} onChange={setCheckOut} placeholder="تسجيل الخروج" disabled={(d) => disablePast(d) || (checkIn ? d <= new Date(checkIn) : false)} className="bg-muted/40 border-border text-sm" />
                </div>
              )}

              {searchTab === "cars" && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                  <CityAutocomplete value={carCity} onChange={setCarCity} placeholder="مدينة الاستلام" inputClassName="bg-muted/40 border-border text-sm text-right" />
                  <DatePickerInput value={carPickup} onChange={setCarPickup} placeholder="تاريخ الاستلام" disabled={disablePast} className="bg-muted/40 border-border text-sm" />
                  <DatePickerInput value={carReturn} onChange={setCarReturn} placeholder="تاريخ الإرجاع" disabled={(d) => disablePast(d) || (carPickup ? d <= new Date(carPickup) : false)} className="bg-muted/40 border-border text-sm" />
                </div>
              )}

              {searchTab === "tours" && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                  <CityAutocomplete value={tourCity} onChange={setTourCity} placeholder="المدينة" inputClassName="bg-muted/40 border-border text-sm text-right" />
                  <DatePickerInput value={tourDate} onChange={setTourDate} placeholder="تاريخ الجولة" disabled={disablePast} className="bg-muted/40 border-border text-sm" />
              ...
              {searchTab === "transfers" && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                  <CityAutocomplete value={transferFrom} onChange={setTransferFrom} placeholder="من (مطار / مدينة)" showCode inputClassName="bg-muted/40 border-border text-sm text-right" />
                  <CityAutocomplete value={transferTo} onChange={setTransferTo} placeholder="إلى (اختياري)" showCode inputClassName="bg-muted/40 border-border text-sm text-right" />
                  <DatePickerInput value={transferDate} onChange={setTransferDate} placeholder="تاريخ الرحلة" disabled={disablePast} className="bg-muted/40 border-border text-sm" />
                </div>
              )}
