...
export default function CarPaymentCallback() {
    ...
    const verify = async () => {
      ...
      try {
        const { data, error } = await supabase.functions.invoke("moyasar-verify", {
          body: { session_id: sessionId, payment_id: paymentId, flow: "car" },
        });
        if (error) throw error;
        if (data?.success) {
          setStatus("success");
          setBookingRef(data.booking_id || data.confirmation_id || "");
          setMessage("تم تأكيد حجز السيارة بنجاح!");
        } else {
          setStatus("failed");
        ...
        {status === "success" && (
          <>
            ...
            <div className="flex gap-3">
              ...
              <Button variant="gold" className="flex-1" onClick={() => navigate("/cars")}><Car className="w-4 h-4 ml-1" /> حجز آخر</Button>
            </div>
          </>
        )}
        {status === "failed" && (
          <>
            ...
            <p className="text-muted-foreground">{message}</p>
            <Button variant="outline" onClick={() => navigate("/cars")}>العودة لتأجير السيارات</Button>
          </>
        )}
      </div>
    </div>
