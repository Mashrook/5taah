...
export default function Hotels() {
  ...
  const [liveCity, setLiveCity] = useState(urlParams.get("city") || "");
  const [checkIn, setCheckIn] = useState(urlParams.get("checkIn") || "");
  const [checkOut, setCheckOut] = useState(urlParams.get("checkOut") || "");
  const [adults, setAdults] = useState(Number(urlParams.get("guests")) || 2);
  ...
  useEffect(() => {
    ...
    if (cityParam && checkInParam && checkOutParam) {
      ...
      const cityCode = resolveIata(cityParam);
      setSearching(true);
      setViewMode("live");
      searchHotelsByCity(cityCode)
              ...
              <div>
                ...
                <DatePickerInput
                  ...
                  disabled={(date) => {
                    ...
                    // prevent selecting same day / before check-in
                    const y = date.getFullYear();
                    const m = date.getMonth();
                    const d = date.getDate();
                    const local = new Date(y, m, d);
                    const [yy, mm, dd] = checkIn.split("-").map(Number);
                    const checkInLocal = new Date(yy, (mm || 1) - 1, dd || 1);
                    return local <= checkInLocal;
                  }}
                />
              </div>
              <div>
                ...
                <div className="bg-muted/30 border border-border rounded-lg px-3 h-10 flex items-center gap-2">
                  ...
                  <select value={adults} onChange={(e) => setAdults(Number(e.target.value))}
                    title="عدد الضيوف" className="bg-transparent w-full text-sm text-foreground outline-none text-right">
...
function FeaturedHotelsSection() {
  ...
  useEffect(() => {
    supabase.from("hotels").select("*").eq("is_active", true).order("sort_order").limit(6)
