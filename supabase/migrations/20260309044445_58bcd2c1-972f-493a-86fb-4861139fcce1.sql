...
-- Update guest_bookings RLS to allow transfer booking type
...
FOR INSERT
WITH CHECK (
  booking_type = ANY (ARRAY['flight'::text, 'hotel'::text, 'car'::text, 'tour'::text, 'transfer'::text])
  AND total_price >= 0
  AND currency IS NOT NULL
  AND length(currency) > 0
);

-- Update payment_sessions RLS to allow transfer flow
